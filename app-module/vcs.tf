...
#     branch       = local.argocd_vcs_configuration.branch
#   }

#   provisioner "local-exec" {
#     command = <<-EOT
#       # Detach VCS repo
#       curl -s -H "Authorization: Bearer ${var.tfe_token}" \
...
#         -X PATCH \
#         "https://app.terraform.io/api/v2/workspaces/${tfe_workspace.app.id}" \
#         -d '{"data":{"type":"workspaces","attributes":{"vcs-repo":{"identifier":"${local.hyperspace_org_name}/Hyperspace-terraform-module","branch":"${local.argocd_vcs_configuration.branch}","oauth-token-id":null,"github-app-installation-id":null}}}}'
#     EOT
#   }
