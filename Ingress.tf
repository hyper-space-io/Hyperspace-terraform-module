...
resource "kubernetes_ingress_v1" "nginx_ingress" {
  ...
  metadata {
    ...
    namespace = "ingress"
    annotations = merge({
      "alb.ingress.kubernetes.io/certificate-arn"          = local.create_acm ? (each.key == "internal" ? module.internal_acm.acm_certificate_arn : module.external_acm.acm_certificate_arn) : ""
      ...
      "alb.ingress.kubernetes.io/actions.ssl-redirect" = (each.key == "internal" && module.internal_acm.acm_certificate_arn != "") || (each.key == "external" && var.create_public_zone && var.domain_name != "") ? jsonencode({
        Type = "redirect"
        RedirectConfig = {
          Protocol   = "HTTPS"
          Port       = "443"
      ...
      "alb.ingress.kubernetes.io/listen-ports" = (each.key == "internal" && module.internal_acm.acm_certificate_arn != "") || (each.key == "external" && var.create_public_zone && var.domain_name != "") ? jsonencode([
        { HTTP = 80 },
        { HTTPS = 443 }
      ]) : jsonencode([{ HTTP = 80 }])
